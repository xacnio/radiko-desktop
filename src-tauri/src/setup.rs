...
pub fn generate_default_cover(app: &App) {
    if let Ok(cache_dir) = app.path().app_cache_dir() {
        ...
        let cover_str = format!("file:///{}", default_cover_path.to_string_lossy().replace('\\', "/"));
...
pub fn setup_html_splash(app: &mut App, theme: Option<&str>) {
    ...
    let bg_color = if is_light { (245, 245, 245, 255) } else { (13, 13, 13, 255) };
...
pub fn await_frontend_and_close_splash(
    ...
    if poll_win.is_none() {
        if let Some(s) = splash_handle { s.close(); }
        return;
    }
