                ...
                if url_str.starts_with("/proxy?") || url_str.starts_with("/stream?") {
                    ...
                    for pair in query.split('&') {
                        ...
                        if parts.next() == Some("url") {
                             if let Some(val) = parts.next() {
                                 target_url = urlencoding::decode(val).unwrap_or(val.into()).to_string();
                             }
                        }
                    }
                    ...
                    match client.get(&target_url).send() {
                        Ok(resp) => {
                            ...
                            let is_shoutcast = h.get("Server").and_then(|v| v.to_str().ok()).map(|s| s.to_lowercase().contains("shoutcast")).unwrap_or(false);
                            ...
                            if url_str.starts_with("/proxy?") && (is_audio_ct || has_icy || is_shoutcast) && !is_garbage {
                                info!("Detected radio stream via proxy headers: {} (CT: {})", target_url, content_type);
                                crate::commands::send_radio_detect_sidebar(ac.clone(), target_url.clone(), "Proxy Detection".to_string(), "".to_string());
                            }
                        ...
                        Err(e) => {
                            let _ = request.respond(Response::from_string(format!("Proxy Error: {}", e)).with_status_code(502));
                        }
                    }
                } else {
                    let _ = request.respond(Response::from_string("Radiko Proxy Active").with_status_code(200));
                }
            });
        }
    });
