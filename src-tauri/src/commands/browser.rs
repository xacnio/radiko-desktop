...
pub fn internal_layout_link_view(w: &tauri::Window) {
    ...
    if let Some(lv) = w.get_webview("link-view") {
        ...
        let size = w.inner_size().unwrap_or(tauri::PhysicalSize::new(1200, 800));
...
pub async fn open_link_window(app: tauri::AppHandle, url: String) -> Result<(), AppError> {
    ...
    let size = main_win.inner_size().unwrap_or(tauri::PhysicalSize::new(1200, 800));
    ...
    let _link_view = main_win.add_child(
...
#[tauri::command]
pub fn send_radio_detect_sidebar(app: tauri::AppHandle, stream_url: String, name: String, favicon: String) {
    use tauri::Manager;
    ...
    if let Some(sidebar) = app.get_webview("sidebar-view") {
        ...
        let _ = sidebar.eval(format!("if(window.addStream) window.addStream({json_data});"));
    }
}

#[tauri::command]
pub async fn probe_and_add_stream_from_js(app: tauri::AppHandle, state: tauri::State<'_, AppState>, url: String, name: String, favicon: String, _force: bool) -> Result<(), AppError> {
    ...
    tauri::async_runtime::spawn(async move {
        ...
        if let Ok(resp) = reqwest::get(&proxy_url).await {
            let h = resp.headers();
            let ct = h.get(reqwest::header::CONTENT_TYPE)
            ...
            if !is_garbage && (is_audio_ct || has_icy) {
                ...
                if let Ok(result) = scrape_radio_url_internal(proxy_port, url.clone()).await {
                    for s_url in result.stream_urls {
                        send_radio_detect_sidebar(app.clone(), s_url, result.name.clone(), result.favicon.clone());
                    }
                }
            }
        }
...
pub async fn open_radio_browser(app: tauri::AppHandle) -> Result<(), AppError> {
    use tauri::{WebviewBuilder, WebviewUrl, WindowBuilder, Manager};
    ...
    let size = window.inner_size().unwrap_or(tauri::PhysicalSize::new(1200, 700));

    let sidebar_w_phys = (sidebar_w_log * sf) as u32;
    let toolbar_h_phys = (toolbar_h_log * sf) as u32;

    // 2. Add Loading overlay FIRST
    let _loading_view = window.add_child(
        WebviewBuilder::new("loading-view", WebviewUrl::App("/browser-loading.html".into()))
            .background_color(bg.into()),
    ...
    let _toolbar_view = window.add_child(
        WebviewBuilder::new("toolbar-view", WebviewUrl::App("/browser-toolbar.html".into()))
            .background_color(bg.into()),
        tauri::PhysicalPosition::new(0, 0),
    ...
    let _browser_view = window.add_child(
        WebviewBuilder::new("browser-view", WebviewUrl::External("https://www.google.com".parse().unwrap()))
            .incognito(false) 
        ...
        tauri::PhysicalSize::new(size.width.saturating_sub(sidebar_w_phys), size.height.saturating_sub(toolbar_h_phys)),
    ).map_err(|e: tauri::Error| AppError::Settings(e.to_string()))?;

    // 4. Add Sidebar Webview LAST
    let _sidebar_view = window.add_child(
        WebviewBuilder::new("sidebar-view", WebviewUrl::App("/browser-sidebar.html".into()))
            .background_color(bg.into()),
            ...
            let _ = w_detach.run_on_main_thread(move || {
                ...
                unsafe {
                    ...
                    for i in 0..win_count {
                        ...
                        if content_view.is_null() { continue; }
    ...
    fn layout_with_sidebar(w: &tauri::Window) {
        let sf = w.scale_factor().unwrap_or(1.0);
        let size = w.inner_size().unwrap_or(tauri::PhysicalSize::new(1200, 700));
...
unsafe fn detach_inspector_recursive(
    ...
    if view.is_null() || depth > 10 { return; }

    let name = (*view).class().name().to_string();

    let is_wk: bool = msg_send![view, isKindOfClass: wk_class];
    if is_wk {
        info!("[Inspector Detach] Found WKWebView '{}' at depth {}", name, depth);

        let has_attach: bool = msg_send![view, respondsToSelector: sel!(_setInspectorAttachmentView:)];
        info!("[Inspector Detach]   respondsTo _setInspectorAttachmentView: {}", has_attach);
        if has_attach {
