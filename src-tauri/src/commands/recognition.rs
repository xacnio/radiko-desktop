...
pub fn save_identified_song(
            ...
            if let Some(sources) = last_song["sources"].as_array_mut() {
                ...
                if already_has_source {
                    ...
                    if !old_source.contains(&current_source_name) {
                        last_song["source"] = serde_json::Value::String(format!(
                            "{} + {}",
                            old_source, current_source_name
                        ));
                    }
                ...
                last_song["source"] =
                    serde_json::Value::String(format!("{} + {}", old_source, current_source_name));
            }
        } else {
    ...
    let content =
        serde_json::to_string_pretty(&songs).map_err(|e| AppError::Settings(e.to_string()))?;
...
#[tauri::command]
pub fn delete_identified_song(
    app: tauri::AppHandle,
    song_to_delete: serde_json::Value,
) -> Result<(), AppError> {
    let data_dir = app
        .path()
        .app_data_dir()
...
pub async fn identify_song(
    ...
    use crate::player::decoder::{CAPTURED_SAMPLES, CAPTURE_ACTIVE, RECOGNITION_SAMPLE_RATE};
    ...
    for attempt in 1..=max_attempts {
        if attempt == 1 {
            ...
            info!(
                "Music identification: starting capture (attempt {}/{})",
                attempt, max_attempts
            );
        } else {
            let _ = app.emit("identify_phase", "retrying");
            info!(
                "Music identification: retrying capture (attempt {}/{})",
                attempt, max_attempts
            );
        }
        ...
        if samples.is_empty() {
            if attempt == max_attempts {
                ...
                return Err(AppError::InvalidState(
                    "No audio samples captured. Is the radio playing?".into(),
                ));
            } else {
                continue;
            }
        ...
        info!(
            "Music identification: captured {} samples at {}Hz",
            samples.len(),
            sample_rate
        );
        ...
        match shazam_result_thread {
            Ok(rec_result) => {
                info!(
                    "Shazam MATCH FOUND: {} - {}",
                    rec_result.artist_name, rec_result.song_name
                );

                let raw = rec_result.raw_response;
                let images = raw
                    .get("track")
                    .and_then(|t: &serde_json::Value| t.get("images"));
                let cover = images
                    .and_then(|i: &serde_json::Value| {
                        i.get("coverarthq")
                            .or(i.get("coverart"))
                            .or(i.get("background"))
                    })
                ...
                let share_url = raw
                    .get("track")
                    .and_then(|t: &serde_json::Value| t.get("share"))
            ...
            Err(e) => {
                ...
                if error_str.contains("no track found")
                    || error_str.contains("unrecognized")
                    || error_str.contains("not found")
                    || error_str.contains("fingerprint error")
                {
